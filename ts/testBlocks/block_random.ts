...
function moveCircle(circle: HTMLElement, i: number)
{
	const x = i % 5;
...
function moveAll()
{
	...
	AbsOrNot.obj.x += dx;
	if (AbsOrNot.obj.x < 0) AbsOrNot.obj.x = canvasAbsOrNot.width;
	if (AbsOrNot.obj.x > canvasAbsOrNot.width) AbsOrNot.obj.x = 0;
	AbsOrNot.obj.y += dy;
	if (AbsOrNot.obj.y < 0) AbsOrNot.obj.y = canvasAbsOrNot.height;
	if (AbsOrNot.obj.y > canvasAbsOrNot.height) AbsOrNot.obj.y = 0;

	if (dx > 0) AbsOrNot.textX.text = `X: +${AbsOrNot.speed}`;
	else AbsOrNot.textX.text = `X: -${AbsOrNot.speed}`;
	if (dy > 0) AbsOrNot.textY.text = `Y: +${AbsOrNot.speed}`;
	else AbsOrNot.textY.text = `Y: -${AbsOrNot.speed}`;
	redrawAll();
}
...
function getOutputs(tableId: string)
{
	...
	if (tableBody == null) throw new Error("table body not found");
	...
	const outputs: HTMLTableCellElement[] = [];
	tableRows.forEach(el =>
	{
		...
		if (output1 != null) outputs.push(output1);
		if (output2 != null) outputs.push(output2);
	});
